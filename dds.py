...
class dds_header:
    def __init__(self,*args):
        ...
        if not mode:
            ...
            #self.dwHeight=self.dwHeight % 4 + self.dwHeight #correct dimensions
...
class dx10_header:
    def __init__(self,*args):
        ...
        if not mode:
            ...
            self.d3d10_resource_dimension=struct.unpack('<I',data.read(4))[0]
...
class dds_file:
    ...
    def write_texture(self):
        ...
        for i in self.header.dwReserved: t.write(struct.pack('<I',i))
        t.write(struct.pack('<II',self.header.ddspf.dwSize,self.header.ddspf.dwFlags))
        if self.header.ddspf.dwFourCC==('D','X','1','0'):
            temp_fourCC=list(dx10_types[self.header.dwdx10header.dxgi_format])
        else:
            temp_fourCC=self.header.ddspf.dwFourCC
            
        for i in temp_fourCC: t.write(struct.pack('c',i))
        ...
        t.write(struct.pack('<5I',self.header.dwCaps,self.header.dwCaps2,self.header.dwCaps3,self.header.dwCaps4,self.header.dwReserved2))
        #if self.header.ddspf.dwFourCC==('D','X','1','0'):
    ...
    def unswizzle_2k(self):
        ...
        if not self.header.dwMipMapCount: #Force 1 mipmap
            ...
            size=self._get_mipmap_size(self.header.dwMipMapCount)//(4*mode)
        ...
        indexing.seek(0)
        if mode>=1:
            alpha0.write(self.data.read(size)) #write first portion of the alpha
            alpha0.seek(0)
            
            alpha1.write(self.data.read(size)) #write second portion of the alpha
            alpha1.seek(0)
