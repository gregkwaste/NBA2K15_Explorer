...
class GLWidgetQ(QtOpenGL.QGLWidget):
    ...
    def paintGL(self):
        ...
        mult = 5
        GL.glLightfv(GL.GL_LIGHT0, GL.GL_DIFFUSE, (mult * self.scale,
                                                   mult * self.scale, mult * self.scale, mult * self.scale))
        ...
        if self.image and self.info:
            self.renderText(
                0.5, -self.sizeDiv + 0.1, 0.0, "Image Width: " + str(self.tex_width))
            self.renderText(
                0.5, -self.sizeDiv + 0.15, 0.0, "Image Height: " + str(self.tex_height))
            self.renderText(0.5, -self.sizeDiv + 0.2, 0.0,
                            "Compression: " + ''.join(self.image.header.ddspf.dwFourCC))
            self.renderText(0.5, -self.sizeDiv + 0.25, 0.0,
                            "MipMaps: " + str(self.image.header.dwMipMapCount))
        elif self.info:
            self.renderText(
                0.5, -self.sizeDiv + 0.1, 0.0, "Vertex Count: " + str(self.vc))
            self.renderText(
                0.5, -self.sizeDiv + 0.15, 0.0, "Faces Count: " + str(self.fc))
    ...
    def resizeGL(self, width, height):
        ...
        if self.image:
            GL.glViewport(
                (width - f_width) / 2, (height - f_height) / 2, f_width, f_height)
            self.sizeDiv = 1.0
        else:
    ...
    def texture_setup(self, image_data):
        ...
        if not typ:  # normal uncompressed Texture
            ...
            GL.glCompressedTexImage2D(
                GL.GL_TEXTURE_2D, 0, typ, width, height, 0, buf_size, self.image.data.read(buf_size))
